...
for (let i = 0; i < tilesW; ++i) {
    const row = new Array(tilesW);
    conwayGrid.push(row);
    gridCopy.push(new Array(tilesW));
...
function drawFrame() {
    ...
    if (++ticks >= maxTicks) {
        //gif.render will eventually result in gif.on('finished') being emitted
        gif.render();
...
function runTick(grid, gridCopy) {
    for (let i = 0; i < tilesW; ++i) {
        for (let j = 0; j < tilesW; ++j) {
    ...
    for (let i = 0; i < tilesW; ++i) {
        for (let j = 0; j < tilesW; ++j) {
            ...
            for (let i1 = Math.max(i - 1, 0); i1 < i + 2 && i1 < tilesW; ++i1) {
                for (let j1 = Math.max(j - 1, 0); j1 < j + 2 && j < tilesW; ++j1) {
                    if (i1 === i && j1 === j) {
                        continue;
                    } else {
                        neighborsAlive += gridCopy[i1][j1];
                    }
                }
            }

            if (neighborsAlive === 2 || neighborsAlive === 3 && alive) {
                // still alive
            }
            else if (neighborsAlive === 3 && !alive) {
                grid[i][j] = true;
            }
            else {
                grid[i][j] = false;
